...
var counter=0;
var cart = firebase.database().ref().child("cart").child(user);
var itemname;
var itemquantity;
var itemprice;
var sum=0;
...
cart.on("child_added",snap1 => {
    ...
    i++;
     ...
     $("#cartid").append("<tr><td>" +itemname+"</td><td>" +itemquantity+ "</td><td>" +itemprice+ " </td><td>"+minisum+"</td><td> <button type='button' onclick='removefromcart(this.id)' id='idremove+"+i+"' value="+i+" style='background-color=#dc3545;'>&#10060;</button></td></tr>");
...
function proceedToOrder(){
    ...
    var refid =  firebase.database().ref().child("cart").child(user);
    refid.once('value',snap=>{
         if(snap.exists())currentItem1=snap.val();
    });
    var data={
        "user":user,
        "Items":currentItem1,
    ...
    console.log(data);
    placeOrder(user,data,tableId);
    
    
}
function placeOrder(user,order,tableId){
    
      console.log("Order in createOrder=  "+order);
     
        
    firebase.database().ref(`order/${user}`).once("value", snapshot => {
